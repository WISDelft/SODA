...
def loadfile_here(fname):
    f = open('./parameter_L2R/'+fname+".pik")
    data = pickle.load(f)
    f.close()
    return data

def dump_all_data_split_from_2013():
    exp_data_repu,exp_data_repu_norm,act_data, exp_data_mec_naive,exp_data_mec, exp_data_mec_log, parti_data_act, parti_data_exp = load_train_data('train')
    ...
    dumpfile(parti_data_act,'parti_data_act')
    dumpfile(parti_data_exp,'parti_data_exp')
    
    test_data = load_test_data('train')
...
if __name__ == '__main__':
    ...
    model = loadfile_here('alphas_'+sys.argv[1]+'_3Property')#+'_'+str(p))
    
    predicted = answerer_rec_using_trained_model_L2R_3Property(test_data, t_set, exp_dict_uid, exp_dict_score, act_dict_uid, act_dict_score, par_dict_uid, par_dict_score, model, alpha)
