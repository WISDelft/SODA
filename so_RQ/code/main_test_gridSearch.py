...
def loadfile_here(fname):
    f = open('./parameter_gridSearch/'+fname+".pik")
    data = pickle.load(f)
    f.close()
    return data
...
if __name__ == '__main__':
    ...
    model = loadfile_here('log_optimal_alphas_'+sys.argv[1])#+'_'+str(p))
    
    predicted = answerer_rec_using_trained_model(test_data, t_set, exp_dict_uid, exp_dict_score, act_dict_uid, act_dict_score, model, alpha)
