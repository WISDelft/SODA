...
def coff_explore(coffs):
    ...
    for i in range(12):
        print features[coffrank[i][1]]+': '+str(coffrank[i][0]) +';;' + features[coffrank[-i-1][1]]+': '+str(coffrank[-i-1][0])

def feature_remove(qfeatures, term):
    if term != '':
        df = loadfile_flat('effective_df')
        print len(df)
        i = 0
        for t in df:
            if t == term:
...
def classify(train_len=10000):
    train_len = 35318
    qfeatures = numpy.load("qfeatures_extr.pik.npy")
    ...
    labels = numpy.load("qlabels_extr.pik.npy")
    ...
    qfeatures = feature_remove(qfeatures, '4.7.2')
