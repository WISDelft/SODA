...
def check_date(aid, date):
    cur.execute("select aid from sim_qa where aid="+str(aid)+" and TO_CHAR(acreationdate,'YYYY-MM-DD') <'" + date +"'")
    result = cur.fetchone()
    if result == None:
        return False
    else:
...
def get_answer_tag():
    ...
    ats = cur.fetchall()
    for at in ats:
        if not check_date(at[4], '2013-01-01'):
            continue
        aowneruserid = at[0]
...
if __name__ == '__main__':
    #cur.execute('''CREATE TABLE qr_UT_matrix_train (uid INT, tag TEXT , NoAnswers  INT, NoQuestions  INT, NoVotes INT, CONSTRAINT qr_UT_train_key PRIMARY KEY (uid,tag));''')
    #con.commit()
    ...
    for ut in at_NoAnswer:
        ...
        if (ut[0],ut[1]) in at_NoQuestion:
            ...
            nq = 0
        cur.execute("INSERT INTO qr_UT_matrix_train (uid, tag, noanswers, noquestions, novotes) VALUES (%s, %s, %s, %s, %s)", (str(ut[0]),ut[1],str(at_NoAnswer[(ut[0],ut[1])]),str(nq),str(nv)))
    ...
    for ut in at_NoQuestion:
        ...
        cur.execute("INSERT INTO qr_UT_matrix_train (uid, tag, noanswers, noquestions, novotes) VALUES (%s, %s, %s, %s, %s)", (str(ut[0]),ut[1],str(0),str(at_NoQuestion[(ut[0],ut[1])]),str(0)))
